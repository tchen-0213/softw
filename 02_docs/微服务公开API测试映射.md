...
- `04_tests/microservices/public-api-manifest.js` 是机器可读清单；`public-api-coverage.test.js` 会比较代码路由、清单、本文档和 UC01-UC09 流程矩阵，任一空项或新增未映射路由都会使 CI 失败。
...
| product-service | `GET /api/shops/user/:userId` | UC06 | `MS-E2E-TC06` | ALT |
| product-service | `GET /api/shops/:id` | UC06 | `MS-E2E-TC06` | MAIN/ERR |
...
| order-service | `GET /api/orders` | UC04 | `MS-E2E-TC04` | MAIN/ALT/ERR |
...
| order-service | `GET /api/orders/:id` | UC04 | `MS-E2E-TC04` | MAIN/ERR |
| order-service | `POST /api/orders/:id/pay` | UC04 | `MS-E2E-TC04` | MAIN/ERR |
| order-service | `POST /api/orders/:id/cancel` | UC04 | `MS-E2E-TC04` | ALT/ERR |
| order-service | `POST /api/orders/:id/ship` | UC04 | `MS-E2E-TC04` | MAIN/ERR |
...
前端页面回归继续使用 4 条 Playwright 流程组合覆盖 `E2E-TC01` 至 `E2E-TC09`；CI 将 `API_BASE_URL` 指向 `http://127.0.0.1:8081`、将 `E2E_BASE_URL` 指向微服务前端 `http://127.0.0.1:8082`。
...
# 微服务 Compose 启动后，通过 Gateway 执行公开 API 与 UC01-UC09 全量回归
