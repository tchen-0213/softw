...
网关提供 `/health`、`/version`，转发 JWT、Content-Type 和 Idempotency-Key。依赖服务超时或断开时返回 HTTP 503，不返回伪造业务结果。
...
| CI 门禁 | `.github/workflows/ci-cd.yml` 的微服务矩阵分别使用 `softw_users_test`、`softw_catalog_test`、`softw_orders_test`，全部成功后才允许构建镜像和部署 |
