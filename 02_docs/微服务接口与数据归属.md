...
后端已经按业务能力拆分为 3 个可独立构建、测试和部署的业务微服务。API Gateway 只负责统一入口，不计入业务服务数量。微服务版本不再通过单体后端读写业务表，前端可通过网关完成 UC01-UC09。
...
![图 MS-SPLIT 真实微服务划分与数据隔离](images/33-MS-SPLIT.svg)
...
| 公开 API/E2E | `04_tests/microservices/public-api-manifest.js` 与 `api-e2e.test.js` 从网关覆盖 49 项公开业务接口和 UC01-UC09；清单见 `微服务公开API测试映射.md` |
| 可观测性 | `npm run k8s:observe` 查看 Pod、探针、SHA 版本、日志和 Warning Events |
| 失败诊断 | 流水线失败自动上传 describe、Events 和容器日志；本地错误镜像实验已实际恢复 |
| 页面回归 | 微服务前端 `http://localhost:8082` 已运行 Playwright UC01-UC09，4/4 通过 |
| Kind 实测 | `03_devops/2026-08-27-微服务Kubernetes验证记录.md` 记录真实部署、健康检查和跨服务交易结果 |

公开路由清单、UC01-UC09 的 MAIN/ALT/ERR 分支、测试编号和自动防漂移规则统一维护在
