...
本文把 REQ01-REQ12 分配到可部署组件，说明接口、数据和运行结构，并为 UC01-UC12 分别给出组件级业务模型。设计以当前仓库代码为准，不把尚未实现的 Redis、消息队列、Elasticsearch、第三方支付和外部物流写成现有组件。

| 项目 | 设计决定 |
| --- | --- |
| 前端 | React 18、React Router、Redux Toolkit、Axios、Vite |
| 核心业务后端 | Express 单体 API，完整承载 UC01-UC12 |
...
![图 12-ARCH-COMP 系统总组件图](images/png/12-ARCH-COMP.png)
...
![图 MS-SPLIT 微服务划分图](images/png/33-MS-SPLIT.png)
...
![图 21-COMP-SEQ09 UC09 地址维护组件级顺序图](images/png/21-COMP-SEQ09.png)

> 可编辑 Mermaid 源文件：[21-COMP-SEQ09.mmd](models/21-COMP-SEQ09.mmd)

### 5.10 COMP-SEQ10 订单与物流查询

![图 37-COMP-SEQ10 UC10 订单与物流查询组件级顺序图](images/png/37-COMP-SEQ10.png)

> 可编辑 Mermaid 源文件：[37-COMP-SEQ10.mmd](models/37-COMP-SEQ10.mmd)

### 5.11 COMP-SEQ11 取消订单与库存恢复

![图 38-COMP-SEQ11 UC11 取消订单与库存恢复组件级顺序图](images/png/38-COMP-SEQ11.png)

> 可编辑 Mermaid 源文件：[38-COMP-SEQ11.mmd](models/38-COMP-SEQ11.mmd)

### 5.12 COMP-SEQ12 公开店铺与卖家信用

![图 39-COMP-SEQ12 UC12 公开店铺与卖家信用组件级顺序图](images/png/39-COMP-SEQ12.png)

> 可编辑 Mermaid 源文件：[39-COMP-SEQ12.mmd](models/39-COMP-SEQ12.mmd)
...
- UC01-UC12 的完整业务验收以单体 API 为准，微服务目录用于课程拆分和部署对比，不虚构未迁移业务。
