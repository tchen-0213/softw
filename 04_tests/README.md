...
| 层级 | 本机实测结果 | 说明 |
| --- | --- | --- |
| 后端单元/安全/控制器 | 220 通过、0 失败、1 个数据库 API 父入口按设计跳过 | 原有套件 + `backend/tests/regression100.test.js` 的 100 个回归用例 |
| 前端 Vitest | 18 个文件；原有基线 100/100，新增 `frontend/tests/regression100.test.js` 100 个用例 | 新增套件已通过 Node 语法检查；Vitest 运行待本机依赖安装恢复 |
