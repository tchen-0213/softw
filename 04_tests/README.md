...
| 前端 Vitest | 90 | 15 个文件，90/90 通过 |
| 单体 API 集成 | 32 | 由 `test:api` 在 MySQL 环境执行 |
| Playwright E2E | 4 | 覆盖 UC01–UC09 |
| 微服务/网关 | 25 | 13 条服务/公共层测试 + 1 条公开 API 清单测试 + 11 条公开 API E2E |
| **完整自动化合计** | **212** | 成功、异常、边界、权限、路由和跨服务场景分层覆盖 |

本轮前端扩大覆盖范围后实测：语句 94.30%、分支 83.63%、函数 94.33%、行 94.30%，四项均超过 80% 门禁。`npm run verify` 已完成后端 61 条、前端 90 条和 Vite 生产构建验证；网关/公共层 9 条与公开 API 清单 1 条也已在本机通过。
