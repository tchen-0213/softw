...
function validateOrderItems(items) {
  ...
  for (const item of items) {
    if (!isPositiveInteger(item?.productId ?? item?.id)) return '订单商品标识必须是正整数';
    if (!isPositiveInteger(item?.quantity ?? 1)) return '商品数量必须是正整数';
  }
  return null;
}
...
function validateProfilePayload(payload = {}) {
  if (hasOwn(payload, 'email') && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(String(payload.email || ''))) return '邮箱格式不正确';
