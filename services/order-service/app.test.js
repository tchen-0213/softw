...
test('order service persists order snapshots through product service API', async () => {
  ...
  const productServer = await fixture((req, res) => { res.setHeader('content-type', 'application/json'); if (req.url === '/health') return res.end(JSON.stringify({ status: 'ok' })); res.statusCode = 201; return res.end(JSON.stringify({ reservationId: 'order-reservation', status: 'reserved', items: [{ productId: 7, name: '跨服务商品', price: 50, quantity: 2, sellerId: 2, sellerName: 'seller' }] })); });
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${server.address().port}/api/orders`, { method: 'POST', headers: { 'content-type': 'application/json', authorization: `Bearer ${token}`, 'idempotency-key': 'order-reservation' }, body: JSON.stringify({ items: [{ productId: 7, quantity: 2 }], shippingAddress: { address: '测试地址' } }) });
    assert.equal(response.status, 201);
    const order = await response.json();
    assert.equal(Number(order.totalAmount), 100);
