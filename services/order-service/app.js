...
router.get('/api/orders/seller', requireUser, async (req,res,next) => { try { const {page,limit}=parsePaging(req.query); const all=await Order.findAll({order:[['createdAt','DESC']]}); let rows=all.filter(row=>hasSellerItem(row,req.user.id)); if(req.query.status)rows=rows.filter(row=>row.status===req.query.status); const count=rows.length; rows=rows.slice((page-1)*limit,page*limit).map(row=>{const data=orderDto(row);return{...data,items:data.items.filter(item=>Number(item.sellerId)===Number(req.user.id))};}); return res.json({orders:rows,pagination:{total:count,page,limit,pages:Math.ceil(count/limit)}}); } catch(error){return next(error);} });
...
module.exports={app,initialize,sequelize,models:{Order}};
